...
// =============================================================================
// Manus Debug Collector - Vite Plugin (dev only, skipped in Vercel)
// =============================================================================
...
// Only include Manus plugins in dev mode (not in Vercel build)
const isVercel = process.env.VERCEL || process.env.VERCEL_ENV;
const plugins: Plugin[] = [react(), tailwindcss(), jsxLocPlugin()];
if (!isVercel) {
  // Dynamic import for Manus plugins to avoid build errors on Vercel
  try {
    const { vitePluginManusRuntime } = require("vite-plugin-manus-runtime");
    plugins.push(vitePluginManusRuntime());
  } catch {
    // vite-plugin-manus-runtime not available on Vercel
  }
